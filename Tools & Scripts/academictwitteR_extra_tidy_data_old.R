...
#'    \item{"tidy_2"}{Tidy format; additional variables (see vars) are available. Untruncates retweet text and adds indicators for retweets, quotes and replies. Automatically drops duplicated tweets}
#' }
#' #' @param vars
      ...
      if (nrow(raw$sourcetweet.main) > 0) {
        ...
        if (!is.null(vars)) {
          rt <-        # RT and quote entity information. This is incomplete in the tweet.entities for RTs due to truncation and missing for quotes
            ...
            dplyr::filter(id %in% raw$tweet.referenced_tweets[raw$tweet.referenced_tweets$type == "retweeted", "id"]$id) %>% # get retweets only
        ...
        if ("mentions" %in% vars & !is.null(raw$tweet.entities.mentions)) {
          mentions_nonrt <-
            dplyr::left_join(raw$tweet.entities.mentions,
                             dplyr::select(res, tweet_id, sourcetweet_author_id, is_retweet),    # some additional variables are needed to dplyr::filter out quoted users
                             by = "tweet_id") %>%  
          ...
          if (!is.null(rt$entities.mentions)){
            mentions_rt <-
              ...
              dplyr::group_by(id) %>% 
              dplyr::mutate(mentions_username = if ("entities.mentions_username" %in% colnames(.)) list(as.character(entities.mentions_username)) else NA,# mentioned usernames as list per tweet
                            mentions_user_id = if ("entities.mentions_id" %in% colnames(.)) list(as.character(entities.mentions_id)) else NA) %>% # mentioned users' IDs as list per tweet
            ...
            res <- .merge_rt_variables(res) # merge .x and .y variables introduced by RT dataset (seperately for every dataset to account for missing variables)
          }
        }
        if ("ext_urls" %in% vars & !is.null(rt$entities.urls))  {        
          ...
          urls_rt <- urls_rt %>%   
            dplyr::group_by(id) %>% 
            dplyr::mutate(ext_urls = if ("entities.urls_url" %in% colnames(.)) list(entities.urls_url) else NA, # used URLs as list per tweet
                          ext_urls_expanded = if ("entities.urls_expanded_url" %in% colnames(.)) list(entities.urls_expanded_url) else NA,
                          ext_urls_unwound = if ("entities.urls_unwound_url" %in% colnames(.)) list(entities.urls_unwound_url) else NA,
                          ext_urls_title = if ("entities.urls_title" %in% colnames(.)) list(entities.urls_title) else NA,
                          ext_urls_description = if ("entities.urls_description" %in% colnames(.)) list(entities.urls_description) else NA) %>% 
          ...
          res <- .merge_rt_variables(res) # merge .x and .y variables introduced by RT dataset
          }
        if ("hashtags" %in% vars & !is.null(rt$entities.hashtags)) {
          hashtags_rt <-
            ...
            dplyr::group_by(id) %>% 
            dplyr::mutate(hashtags = if ("entities.hashtags_tag" %in% colnames(.)) list(entities.hashtags_tag) else NA) %>% # hashtags as list per tweet
            dplyr::select(id, hashtags) %>% # dplyr::select relevant variables
            dplyr::ungroup() %>% dplyr::distinct(id, .keep_all = TRUE) # drop duplicates introduced by tidyr::unnesting
        ...
        if ("annotations" %in% vars & !is.null(rt$entities.annotations)) {
          annotations_rt <-
            ...
            dplyr::group_by(id) %>% 
            dplyr::mutate(annotation_probability = if ("entities.annotations_probability" %in% colnames(.)) list(as.numeric(entities.annotations_probability)) else NA, # used annotations as list per tweet
                          annotation_type = if ("entities.annotations_type" %in% colnames(.)) list(entities.annotations_type) else NA,
                          annotation_entity = if ("entities.annotations_normalized_text" %in% colnames(.)) list(entities.annotations_normalized_text) else NA) %>% 
        ...
        if ("context_annotations" %in% vars & !is.null(rt$context_annotations)) {
          context_annotations_rt <- 
            ...
            dplyr::group_by(id) %>% dplyr::mutate( # used annotations as list per tweet
              context.domain.id = if ("id" %in% (.[["context_annotations_domain"]] %>% colnames(.))) list(context_annotations_domain$id) else NA, 
              context.domain.name = if ("name" %in% (.[["context_annotations_domain"]] %>% colnames(.))) list(context_annotations_domain$name) else NA,
              context.domain.description = if ("description" %in% (.[["context_annotations_domain"]] %>% colnames(.))) list(context_annotations_domain$description) else NA,
              context.entity.id = if ("id" %in% (.[["context_annotations_entity"]] %>% colnames(.))) list(context_annotations_entity$id) else NA,
              context.entity.name = if ("name" %in% (.[["context_annotations_entity"]] %>% colnames(.))) list(context_annotations_entity$name) else NA,
              context.entity.description = if ("description" %in% (.[["context_annotations_entity"]] %>% colnames(.))) list(context_annotations_entity$description) else NA) %>% 
...
.flat <- function(data_path, output_format = "tidy", vars = c("hashtags", "ext_urls", "mentions", "annotations", "context_annotations")) {
  if (!output_format %in% c("tidy", "raw", "tidy_2")) {
    stop("Unknown format.", call. = FALSE)
  }
...
.merge_rt_variables <- function(x) {
  variables <- dplyr::select(x, tidyselect::ends_with(".x")) %>% colnames(.) %>% stringr::str_extract(".*(?=.x)")
  if (!purrr::is_empty(variables)) {
    for (i in 1:length(variables)) {
      if (!is.na(variables[i])) {
        x <- x %>% dplyr::mutate(!!as.name(variables[i]) := ifelse(is_retweet == T,   
                                                            !!as.name(paste0(variables[i],".y")),          
                                                            !!as.name(paste0(variables[i],".x"))))
      }
    }
    x <- x %>% dplyr::select(!tidyselect::ends_with(c(".x", ".y")))
  }  
  return(x)
}

# to do: behaviour for quotes is currently inconsistent. if there are no entities in the quoted tweets of a dataset, 
